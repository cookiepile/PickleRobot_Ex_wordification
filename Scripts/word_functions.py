...
def all_wordifications(input):
    ...
    number_without_hyphen, words, word_index, alphabet_index = word_numbers(input)

    # create empty list to store all possible wordified numbers
    all_wordified_phone_numbers=[]

    # create wordified phone number for each combination of english found as done in number_to_words function
    for word in range(len(words)):
        answer = str(number_without_hyphen[0:alphabet_index[word_index[word]]]) + words[word] + number_without_hyphen[
                                                                            alphabet_index[word_index[word]] - 1 + len(words[word]) + 1:len(
                                                                                number_without_hyphen)]
        answer_with_hyphen = []
        s_index = 0

        # formatting wordified phone numbers by adding hyphen
        for s in answer:
            answer_with_hyphen.append(s)
            if s_index == 0 and (s.isdigit() or answer[s_index + 1].isdigit()):
                answer_with_hyphen.append('-')
        ...
        # add each wordified phone number in a list of all_wordifications_phone_numbers
        all_wordified_phone_numbers.append(wordified_phone_number)

    return all_wordified_phone_numbers